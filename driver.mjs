...
const COMMANDS = {
  async launch() {
    ...
    wins.forEach(w => console.log(' ', w.url()));
    page = wins.find(w => !w.url().startsWith('devtools://')) ?? await app.firstWindow();
    console.log('launched. active page:', page.url());
  },
  ...
  async windows() {
    ...
    } catch(e) { console.log('(webContents eval error:', e.message + ')'); }
  },
...
console.log('DarkBrowser driver — "help" for commands, "launch" to start');
rl.prompt();
